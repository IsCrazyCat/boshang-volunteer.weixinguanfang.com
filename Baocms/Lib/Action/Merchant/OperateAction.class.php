...
class OperateAction extends CommonAction {
    ...
    public function index() {
		...
		//评价
		$counts['shop_dianping'] = (int) D('Shopdianping')->where(array('shop_id' => $this->shop_id,'closed' => 0))->count();//总组织/团体点评
		$counts['totay_shop_dianping'] = (int) D('Shopdianping')->where(array('shop_id' => $this->shop_id,'closed' => 0,'create_time' => array(array('ELT', NOW_TIME),array('EGT', $bg_time),), 'status' => array('EGT', 0),))->count();//今日组织/团体点评
		...
		//组织/团体预约
		$counts['shopyuyue'] = (int) D('Shopyuyue')->where(array('shop_id' => $this->shop_id,))->count();//总组织/团体预约数量
