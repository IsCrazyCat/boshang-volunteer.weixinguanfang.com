...
class BookingAction extends CommonAction {
   ...
   public function index() {
    ...
    public function ding($shop_id) {
        ...
        if(!$shop_id = (int)$shop_id){
			...
			if(empty($get_booking_shop_cfg)){
				$this->error('该组织/团体没有配置预订时间，暂时无法预约，请联系组织/团体');
			}
    ...
    public function orderCreate($shop_id){
		...
		if (!$shop_id = (int)$shop_id) {
            $this->ajaxReturn(array('status'=>'error','msg'=>'该组织/团体不存在'));
        }
        if (!$detail = $Booking->find($shop_id)) {
            $this->ajaxReturn(array('status'=>'error','msg'=>'该组织/团体不存在'));
        }
        if ($detail['audit'] != 1||$detail['closed']!=0) {
            $this->ajaxReturn(array('status'=>'error','msg'=>'组织/团体已删除或未审核'));
        }
		...
		if (empty($ding_time)) { 
			...
			$this->ajaxReturn(array('status'=>'error','msg'=>'组织/团体已经打烊或者选择时间不正确'));
		}
	...
	public function rooms(){
		...
		$ding_num = htmlspecialchars($_COOKIE['ding_num']); 
		$room = D('Bookingroom');
		$room_detail = $room->getrooms($shop_id,$ding_date,$ding_time,$ding_num );//组织/团体ID，预约日期，预约时间，预约人数
