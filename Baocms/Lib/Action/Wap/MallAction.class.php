   ...
   public function get_spec($goods_id){
 	...
 	foreach($goods_ids as $v){
       ...
       if (empty($goods)) {
		...
		$num[$val['goods_id']] = $num[$val['goods_spec']];
            ...
            $total_canuserintegral[$val['shop_id']] += $canuserintegral; //不同组织/团体可使用积分
            $total_price[$val['shop_id']] += $price; //不同组织/团体的总价格
			$express_price[$val['shop_id']] += $order_express_price; //不同组织/团体总运费
            $mm_price[$val['shop_id']] += $mobile_fan;  //不同组织/团体的手机下单立减
    ...
    public function paycode2(){
        ...
        if ($code == 'wait') {
			...
			D('Sms')->mallTZshop($order_ids);//用户下单通知组织/团体
			D('Order')->combination_goods_print($order_ids);//多组织/团体订单打印
    ...
    public function pay2(){
        ...
        if ($code == 'wait') {
			...
			D('Sms')->mallTZshop($order_id);//用户下单通知组织/团体
