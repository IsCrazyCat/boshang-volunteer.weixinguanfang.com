...
class ShopAction extends CommonAction{
    ...
    public function detail()
    {
        ...
        $this->seodatas['cate_business'] = $this->bizs[$detail['business_id']]['business_name'];//街道
    ...
    public function favorites(){
        ...
        $shop_id = (int) $this->_get('shop_id');
        if (!($detail = D('Shop')->find($shop_id))) {
            $this->baoError('没有该组织/团体');
        }
        if ($detail['closed']) {
            $this->baoError('该组织/团体已经被删除');
        }
        if (D('Shopfavorites')->check($shop_id, $this->uid)) {
            $this->baoError('您已经关注过该组织/团体了！');
        }
...
//        }
        if ($this->isPost()) {
            ...
            if ($words = D('Sensitive')->checkWords($details)) {
                $this->baoError('组织/团体介绍含有敏感词：' . $words, 2000, true);
            }
    ...
    private function createCheck()
    {
        ...
        if (empty($data['shop_name'])) {
            $this->baoError('组织/团体名称不能为空', 2000, true);
        }
        $data['lng'] = htmlspecialchars($data['lng']);
        $data['lat'] = htmlspecialchars($data['lat']);
        if (empty($data['lng']) || empty($data['lat'])) {
            $this->baoError('组织/团体坐标需要设置', 2000, true);
        }
        ...
        if (empty($data['business_id'])) {
            $this->baoError('街道不能为空', 2000, true);
        }
        ...
        if (!isImage($data['photo'])) {
            $this->baoError('请上传正确的组织/团体图片', 2000, true);
        }
        ...
        if (!empty($detail)) {
            $this->baoError('您已经是组织/团体了', 2000, true);
        }
        $data['recognition'] = 1;
		$data['is_pei'] = 1;
    ...
    public function yuyue2()
    {
        ...
        if (!($detail = D('Shop')->find($shop_id))) {
            $this->ajaxReturn(array('status' => 'error', 'msg' => '没有该组织/团体'));
        }
        if ($detail['closed']) {
...
//            }
            ...
            if ($obj->add($data)) {
                ...
                D('Email')->sendMail('pc_email_recognition', $pc_email_recognition, '你好，管理员：有客户认领组织/团体了！', array(
					'shop_name' => $shop_name, 
					'name' => $data['name'], 
					'mobile' => $data['mobile'], 
					'content' => $data['content']
