...
class EleAction extends CommonAction{
    ...
    public function order()
    {
		...
		if (false == D('Shop')->check_shop_user_id($shop_id,$this->uid)) {//不能购买自己家的产品
			 $this->ajaxReturn(array('status' => 'error', 'msg' => '对不起，您是该组织/团体管理员，无法购买哦'));
		}
        if (!$shop['is_open']) {
            $this->ajaxReturn(array('status' => 'error', 'msg' => '组织/团体已经打烊，实在对不住客官'));
        }
		$busihour = $this->closeshopele($shop['busihour']);
		 if ($busihour == 1) {
            $this->ajaxReturn(array('status' => 'error', 'msg' => '当前时间段组织/团体正在休息，请稍后再来，谢谢'));
        }
