...
class WeixinAction extends CommonAction {
    ...
    public function tuan() {
		...
		if(empty($worker)){
			$this->error('您不属于任何一个组织/团体的授权员工，无权进行管理！', U('index/index'));
		}
			...
			//这次更新保证了更新的结果集
			 D('Sms')->tuan_TZ_user($data['code_id']);//发短信，先发再处理逻辑
				...
				if (!empty($data['price'])) {
					...
					D('Users')->Money($shop['user_id'], $data['settlement_price'], '组织/团体抢购结算:' . $data['order_id']);//商户资金增加
					//抢购返还积分给组织/团体用户
					if(!empty($data['real_integral'])){
						$config = D('Setting')->fetchAll();
						if($config['integral']['tuan_return_integral'] == 1){
							D('Users')->return_integral($shop['user_id'], $data['real_integral'] , '抢购用户消费积分返还给组织/团体');
						}
					}
    ...
    public function coupon() {
		...
		if(empty($worker)){
			$this->error('您不属于任何一个组织/团体的授权员工，无权进行管理！', U('worker/index/index'));
		}
		if(empty($worker['status']) || $worker['status'] !=1 ){
