...
class ShoprecognitionAction extends CommonAction{
    ...
    public function audit($recognition_id = 0)
    {
            ...
            if (!empty($user_ids['email'])) {
                D('Email')->sendMail('email_recognition', $user_ids['email'], '组织/团体认领成功的通知', array(
					'shop_name' => $shops['shop_name'], 
					'name' =>$user_ids['name'], 
					'mobile' => $user_ids['mobile'], 
					'content' => $detail['content'], 
            ...
            $obj_shop->save(array('shop_id' => $detail['shop_id'], 'recognition' => 1,'user_id' => $detail['user_id']));//更改组织/团体认领状态
