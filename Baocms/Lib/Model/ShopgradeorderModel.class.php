...
class ShopgradeorderModel extends CommonModel {
	...
	public function shop_pay_grade($grade_id,$shop_id){
		...
		$shop_grade = $obj->find($grade_id);//准备购买的组织/团体等级
		$old_shop_grade = $obj->find($Shop['grade_id']);//当前组织/团体的等级
		if(empty($shop_grade)){
			$this->error = '您购买的等级不存在或者被删除了';
			return false;
		}elseif($Shop['grade_id'] == $grade_id){
			$this->error = '购买的等级跟您的组织/团体等级一致，无法购买';
			return false;
	   ...
	   if($order_id){
			if (false !== D('Users')->addMoney($users['user_id'], -$shop_grade['money'], '提升组织/团体等级【' . $shop_grade['grade_name'] . '】扣费成功')) {
