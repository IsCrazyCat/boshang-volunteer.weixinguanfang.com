  ...
  //更新退款库存
   public function del_goods_num($order_id) {
	...
	public function goods_print($order_id,$address_id) {	
			...
			$shop_print = D('Shop')->where(array('shop_id'=> $order['shop_id']))->find();//组织/团体信息
			...
			$msg .= '********************************\r';
			$msg .= '组织/团体名称：' . $shop_print['shop_name'] .'\r';
   ...
   //商城万能打印接口
                ...
                if ($order['is_daofu'] == 0) {
					...
					if(!empty($order['use_integral'])){
						if(!empty($config['integral']['mall_return_integral'])){
							D('Users')->return_integral($shop['user_id'], $order['use_integral'] , '商城用户积分兑换返还给组织/团体');
						}
	...
	public function implemented_refund($order_id){
		...
		if (!empty($order_id)) {
			...
			D('Weixintmpl')->weixin_shop_confirm_refund_user($order_id,2);//家政组织/团体确认退款，传订单ID跟类型
        }else{
			return false;	
	   }
	   return TRUE;
