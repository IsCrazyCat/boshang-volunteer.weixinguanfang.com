        ...
        if (!empty($logs) && !$logs['is_paid']) {
            ...
            if (D('Paymentlogs')->save($data)) { //总之 先更新 然后再处理逻辑  这里保障并发是安全的
                ...
                if ($logs['type'] == 'appoint') {//家政购买
					...
					D('Sms') -> sms_appoint_TZ_shop($logs['order_id']);//家政短信通知组织/团体
                    return true;
					...
					D('Sms') -> breaksTZshop($order['order_id']);//发送短信给组织/团体
					...
					D('Sms') -> tuanTZshop($tuan['shop_id']);//发送短信通知组织/团体
					...
					D('Sms') -> eleTZshop($logs['order_id']);//通知组织/团体
					...
					D('Sms')->sms_hotel_shop($logs['order_id']);//短信通知酒店组织/团体
                    return true;
                    ...
                    if (empty($logs['order_id']) && !empty($logs['order_ids'])) {//合并付款
                        ...
                        D('Sms')->mallTZshop($order_ids); //通知组织/团体
                        ...
                        D('Sms')->mallTZshop($logs['order_id']);//通知组织/团体
